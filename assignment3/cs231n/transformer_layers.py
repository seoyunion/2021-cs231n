...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        i = torch.arange(0, max_len).reshape(1, max_len, 1)
        j_even = (10000**(-torch.arange(0, embed_dim, 2)/embed_dim)).reshape(1,1,embed_dim//2)
        j_odd = (10000**(-(torch.arange(1, embed_dim, 2)-1)/embed_dim)).reshape(1,1,embed_dim//2)
        pe[:,:,1::2] = torch.cos(i * j_odd)
        pe[:,:,::2] = torch.sin(i * j_even)
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0.1):
        ...
        self.scalar = (embed_dim / num_heads)**(-.5)
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        # softmax(N,H,S,D//H x N,H,D//H,T) = N,H,S,T
        # N,H,S,T x S,T
        # N,H,S,T x N,H,T,D//H = N,H,S,D//H -> N,S,H,D//H -> N,S,D
        # N,S,D x D,D
        H = self.num_heads
        
        scores = self.scalar * self.query(query).reshape(N,S,H,D//H).transpose(1,2).matmul(
          self.key(key).reshape(N,T,H,D//H).permute(0,2,3,1)
        )

        if attn_mask is not None:
          scores.masked_fill_(attn_mask == 0, float('-inf'))

        att = self.dropout(self.softmax(scores))
        
        output = att.matmul(
          self.value(value).reshape(N,T,H,D//H).transpose(1,2)
        ).transpose(1,2).reshape(N,S,D)
